...
#define LOGI(...) ((void)__android_log_print(ANDROID_LOG_INFO, "LIA threaded_app", __VA_ARGS__))
#define LOGE(...) ((void)__android_log_print(ANDROID_LOG_ERROR, "LIA threaded_app", __VA_ARGS__))

//#ifndef NDEBUG
#  define LOGV(...)  ((void)__android_log_print(ANDROID_LOG_VERBOSE, "LIA threaded_app", __VA_ARGS__))
//#else
//#  define LOGV(...)  ((void)0)
//#endif
...
void ANativeActivity_onCreate(ANativeActivity* activity, void* savedState,
  ...
  { JNIEnv *env=0;
    ...
    if (cls == NULL) LOGI("cls NULL\n");
    jmethodID method = (*env)->GetMethodID(env,cls,"getIntent","()Landroid/content/Intent;");
    if (method == NULL) LOGI("method NULL\n");
    jobject *intent = (*env)->CallObjectMethod(env,activity->clazz,method);
    if (intent == NULL) LOGI("intent NULL *****\n");
    else LOGI("intent OK *****\n");
    // Bundle extras = intent.getExtras();
    cls = (*env)->FindClass(env,"android/content/Intent");
    method = (*env)->GetMethodID(env,cls,"getExtras","()Landroid/os/Bundle;");
    jobject *extras = (*env)->CallObjectMethod(env,intent,method);
    jint Id;
    if (extras == NULL) {
      LOGI("extras NULL *****\n");
      Id = 0;
      ...
      cls = (*env)->FindClass(env,"android/os/Bundle");
      method = (*env)->GetMethodID(env,cls,"getInt","(Ljava/lang/String;I)I");
      jstring EXTRA_APPWIDGET_ID = (*env)->NewStringUTF(env,"appWidgetId");
      Id = (*env)->CallIntMethod(env,extras,method,EXTRA_APPWIDGET_ID,0);
    };
    LOGI("Id = %d *****\n",Id);
    // Intent resultValue = new Intent(); => resultValue = intent.clone(); 
